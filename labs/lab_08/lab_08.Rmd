...
**A1. I would say there are very few similarities and there are major differences between the charts here and in the pre lab. The most obvious difference is that December has the most calls in this data set but the second least of any month in the pre lab/Baltimore data.**
...
**A2. It increases throughout the year with some minor ups and downs but hits its peak midway through the year with a potential outlier of over 40 calls in a week**
...
pg_calls_week |> 
  ...
  labs(
    title="Prince George's County 911 Overdose Calls By Week",
    y = "number of calls",
    x = "week"
  )
...
md_median_income_zip <- get_acs(geograph = 'zcta', variables = 'B19013_001', state = 'MD', year =2019)
...
ggplot(pg_calls_with_income, aes(x=estimate,y-total_calls))+geom_point()+
  labs(
    title = 'Wealthier areas of PG county have fewer overdose calls',
    x = 'Median income',
    y = 'Total calls'
  )+
  geom_text(aes(x=estimate+1,y=total_calls, label= zipcode), check_overlap = TRUE, size=2,angle=45)
```
