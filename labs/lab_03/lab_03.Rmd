...
A2: The year with most spending was in 2020. I notice that for the most part, it increases on a yearly basis. I wonder if the pandemic had any correlation with the increased spending in 2020
...
A3.There definitely needs to be a substantial amount of cleaning done. When looking at the code, there are many different vendors that are listed several times that are likely the same one — some of this is done by typos (aerosol.. & aersol), some by not listing the full name (cannon design vs. cannon design inc) and also not including a hyphen (colimore thoemke vs. colimore-thoemke). I don't know exactly how to filter it, but maybe there's a way to combine the spending of multiple vendors if their name matches to a certain percentage (like if the characters are 90+% aligned.
...
A4: It definitely seems like the top year, which was also the first year, was the outliar. Since then it's drastically decreased and did so almost right away, although the spending has increased slightly after reaching low amounts in 2014-18. It's also worth noting that several years are missing here.
