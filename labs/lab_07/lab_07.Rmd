...
```{r}
census_api_key("9c74e3f85ddabb809bb6b43d3785a36a916de28e", install=TRUE, overwrite=TRUE)
```
...
**Q1** What is the Maryland jurisdiction with the lowest median age, according to the ACS 2021 5-year estimates? You will need to find the variable for median age first (search for it in the 'concept' column at the data frame we just created). **A1 Baltimore City**
...
**A3 20877 has the highest rate (1.7) and 20707 has the lowest on the list (0.03), however there are some zip codes that have 0 and aren't on the list at all.**
...
montgomery_zip_overdose <- montgomery_overdose_calls |> 
  ...
  group_by(zip) |> summarize(total_calls = n()) |>
...
**A4 I couldn't find any overdose data on the website for some reason, though if there is a strong pattern it's possible that a story is there to examine why it's affecting certain groups more than others.**
...
**A5** I think this gives more confidence in my answers to Q3 because both the top 2 highest and bottom 2 lowests are same for both the highest and lowest populations based on MOE.

```{r}
montgomery_pop_overdose <- moco_pop_overdose |>
  mutate(minimum_pop = (estimate-moe)) |>
  mutate(maximum_pop = (estimate+moe)) |>
  mutate(minimum_per_1k = (total_calls/minimum_pop)*1000) |>
  mutate(maximum_per_1k = (total_calls/maximum_pop)*1000)
```
