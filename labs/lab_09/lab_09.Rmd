...
**A2.**<https://www.datawrapper.de/_/faQOT/>

I think the story is that the voting types are starting to resemble
pre-COVID voting types instead of the skewed 2020 version.
...
**A3. It's hard for me to identify a full story based on this, but one
potential option could be analyzing the common denominators of the zips
with the highest counts to figure out why they may be so high**

<https://www.datawrapper.de/_/Z6jsi/>

```{r}
mg_co_911 <- read_csv("data/montgomery_2022_overdoses.csv")

mg_co_911_zip <- mg_co_911 |> 
  group_by(zip) |> 
  summarize(count = n()) |>
  mutate(zip_pct = (count/sum(count))*100)

write_csv(mg_co_911_zip, 'data/montgomery_2022_overdoses_zip.csv')
```
