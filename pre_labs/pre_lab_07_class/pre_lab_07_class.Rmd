...
```{r echo=FALSE}
census_api_key("9c74e3f85ddabb809bb6b43d3785a36a916de28e", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Nevada has the largest change at about 35 percent**
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Household income by county in Maryland in USD**
...
Is the difference between Calvert and Montgomery significant? **We don't know if there is much of a difference if at all between Calvert and Montgomery because there is overlap between them on the chart, meaning that considering the MOE, either one could be higher than the other**

Is the difference between Howard and everyone else significant? **I would say so because it is definitely the highest county even if you subtract its MOE while adding the second-highest county's MOE**
